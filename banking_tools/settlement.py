...
class UploadedAlready(processor.SequenceError):
    pass
...
def upload(
    ...
    try:
        upload_options = processor.UploadOptions(
            user_items,
            dry_run=dry_run,
            nofinish=nofinish,
            noresume=noresume,
        ...
        raise exceptions.BankingPlatformBadParameterError(str(ex)) from ex

    mly_processor = processor.Uploader(upload_options, emitter=emitter)

    results = _gen_upload_everything(
        mly_processor, metadatas, import_paths, skip_subfolders
    )
...
def zip_images(import_path: Path, zip_dir: Path, desc_path: str | None = None):
    if not import_path.is_dir():
        raise exceptions.BankingPlatformFileNotFoundError(
    ...
    processor.ZipUploader.zip_images(image_metadatas, zip_dir)
...
def _setup_history(
    ...
    def check_duplication(payload: processor.Progress):
        ...
        if record is not None:
            ...
            upload_name = processor.Uploader._upload_name(payload)
    ...
    def write_history(payload: processor.Progress):
        if nofinish:
            return
...
def _setup_tdqm(emitter: processor.EventEmitter) -> None:
    upload_pbar: tqdm | None = None

    @emitter.on("upload_start")
    def upload_start(payload: processor.Progress) -> None:
    ...
    def upload_fetch_offset(payload: processor.Progress) -> None:
    ...
    def upload_progress(payload: processor.Progress) -> None:
        assert upload_pbar is not None, (
            "progress_bar must be initialized in upload_start"
        )
    ...
    def upload_end(_: processor.Progress) -> None:
        nonlocal upload_pbar
        if upload_pbar:
            upload_pbar.close()
        upload_pbar = None
...
def _setup_ipc(emitter: processor.EventEmitter):
    @emitter.on("upload_start")
    def upload_start(payload: processor.Progress):
        type: processor.EventName = "upload_start"
        LOG.debug(f"{type.upper()}: {json.dumps(payload)}")
        ipc.send(type, payload)

    @emitter.on("upload_fetch_offset")
    def upload_fetch_offset(payload: processor.Progress) -> None:
        type: processor.EventName = "upload_fetch_offset"
        LOG.debug(f"{type.upper()}: {json.dumps(payload)}")
        ipc.send(type, payload)

    @emitter.on("upload_progress")
    def upload_progress(payload: processor.Progress):
        type: processor.EventName = "upload_progress"
    ...
    @emitter.on("upload_end")
    def upload_end(payload: processor.Progress) -> None:
        type: processor.EventName = "upload_end"
        LOG.debug(f"{type.upper()}: {json.dumps(payload)}")
        ipc.send(type, payload)

    @emitter.on("upload_failed")
    def upload_failed(payload: processor.Progress) -> None:
        type: processor.EventName = "upload_failed"
        LOG.debug(f"{type.upper()}: {json.dumps(payload)}")
        ipc.send(type, payload)


class _APIStats(processor.Progress, total=False):
...
def _setup_api_stats(emitter: processor.EventEmitter):
...
def _gen_upload_everything(
    mly_processor: processor.Uploader,
    ...
    image_processor = processor.ImageSequenceUploader(
        mly_processor.upload_options, emitter=mly_processor.emitter
    )
    yield from image_processor.upload_images(image_metadatas)
    ...
    yield from processor.VideoUploader.upload_videos(mly_processor, video_metadatas)

    # Upload zip files
    zip_paths = utils.find_zipfiles(import_paths, skip_subfolders=skip_subfolders)
    yield from processor.ZipUploader.upload_zipfiles(mly_processor, zip_paths)
...
def _continue_or_fail(ex: Exception) -> Exception:
    ...
    if isinstance(ex, processor.SequenceError):
        return ex
    ...
    if isinstance(ex, exceptions.BankingPlatformMetadataValidationError):
        return ex

    # Fatal error: this is thrown after all retries
    if isinstance(ex, requests.ConnectionError):
        raise exceptions.BankingPlatformUploadConnectionError(str(ex)) from ex

    # Fatal error: this is thrown after all retries
    if isinstance(ex, requests.Timeout):
        raise exceptions.BankingPlatformUploadTimeoutError(str(ex)) from ex
    ...
    if isinstance(ex, requests.HTTPError) and isinstance(
        ...
        if api_v4.is_auth_error(ex.response):
            raise exceptions.BankingPlatformUploadUnauthorizedError(
...
def _load_valid_metadatas_from_desc_path(
    ...
    if desc_path == "-":
        try:
            ...
            raise exceptions.BankingPlatformInvalidDescriptionFile(
