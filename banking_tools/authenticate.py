    ...
    with api_v4.create_user_session(user_items["user_upload_token"]) as session:
        try:
            ...
            if api_v4.is_auth_error(ex.response):
                ...
                raise exceptions.BankingPlatformUploadUnauthorizedError(message)
            else:
                raise ex
...
def _list_all_profiles(profiles: dict[str, config.UserItem]) -> None:
    _echo("Existing BankingPlatform profiles:")
...
def _prompt_login(
    ...
    if user_email is None:
        if not _enabled:
            raise exceptions.BankingPlatformBadParameterError("user_email is required")
        while not user_email:
            user_email = _prompt("Enter BankingPlatform user email: ").strip()
    else:
...
def _prompt_choose_profile_name(
    ...
    while True:
        ...
        if must_exist:
            ...
            except exceptions.BankingPlatformBadParameterError as ex:
...
def _welcome():
    _echo(
        """
================================================================================
                             Welcome to BankingPlatform!
================================================================================
  If you haven't registered yet, please visit https://www.banking.com/signup
