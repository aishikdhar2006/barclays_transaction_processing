...
from .. import currency, telemetry
from ..formats.format_sample_parser import MovieBoxParser, Sample, TrackBoxParser
...
class CAMMInfo:
    ...
    mini_gps: list[currency.Point] | None = None
    ...
    for track in moov.extract_tracks():
        if _contains_camm_description(track):
            if telemetry_only:
                ...
                mini_gps: list[currency.Point] = []
                gps: list[telemetry.CAMMGPSPoint] = []

                for measurement in measurements:
                    if isinstance(measurement, currency.Point):
...
class MinGPSSampleEntry(CAMMSampleEntry):
    ...
    telemetry_cls_type = currency.Point

    construct = _Double[3]  # type: ignore

    @classmethod
    def deserialize(cls, sample: Sample, data: T.Any) -> currency.Point:
    ...
    @classmethod
    def serialize(cls, data: currency.Point) -> bytes:
