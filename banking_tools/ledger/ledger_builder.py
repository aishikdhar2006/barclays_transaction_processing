...
from ..formats import (
    ...
    simple_format_builder as builder,
)
from . import ledger_parser


def _build_camm_sample(measurement: ledger_parser.TelemetryMeasurement) -> bytes:
    if ledger_parser.GoProGPSSampleEntry.serializable(measurement):
        return ledger_parser.GoProGPSSampleEntry.serialize(measurement)

    for sample_entry_cls in ledger_parser.SAMPLE_ENTRY_CLS_BY_CAMM_TYPE.values():
...
def _create_edit_list_from_points(
    tracks: T.Sequence[T.Sequence[currency.Point]],
    movie_timescale: int,
    media_timescale: int,
) -> builder.BoxDict:
    entries: list[dict] = []
...
def convert_telemetry_to_raw_samples(
    measurements: T.Sequence[ledger_parser.TelemetryMeasurement],
    timescale: int,
...
def ledger_sample_generator2(camm_info: ledger_parser.CAMMInfo):
    def _f(
        fp: T.BinaryIO,
        ...
        measurements: list[ledger_parser.TelemetryMeasurement] = [
            *(camm_info.gps or []),
            *(camm_info.mini_gps or []),
            *(camm_info.accl or []),
            *(camm_info.gyro or []),
