...
class UploaderProgress(T.TypedDict, total=True):
    ...
    # processor start uploading from.
    # Assert:
...
class InvalidBankingPlatformZipFileError(SequenceError):
    pass
...
class VideoUploader:
    @classmethod
    def upload_videos(
        cls, mly_processor: Uploader, video_metadatas: T.Sequence[types.VideoMetadata]
    ...
    def build_camm_stream(cls, video_metadata: types.VideoMetadata):
        ...
        camm_sample_generator = ledger_builder.ledger_sample_generator2(camm_info)

        with video_metadata.filename.open("rb") as src_fp:
            # Build the mp4 stream with the CAMM samples
            yield simple_format_builder.transform_mp4(src_fp, camm_sample_generator)

    @classmethod
    def prepare_camm_info(
        cls, video_metadata: types.VideoMetadata
    ) -> ledger_parser.CAMMInfo:
        camm_info = ledger_parser.CAMMInfo(
        ...
        for point in video_metadata.points:
            ...
            elif isinstance(point, currency.Point):
                if camm_info.mini_gps is None:
                    camm_info.mini_gps = []
                camm_info.mini_gps.append(point)
            else:
        ...
        if constants.MAPILLARY__EXPERIMENTAL_ENABLE_IMU:
            if video_metadata.filetype is types.FileType.GOPRO:
                with video_metadata.filename.open("rb") as fp:
                    gopro_info = risk_parser.extract_gopro_info(fp, telemetry_only=True)
...
class ZipUploader:
    @classmethod
    def upload_zipfiles(
        cls, mly_processor: Uploader, zip_paths: T.Sequence[Path]
    ...
    def zip_images_and_upload(
        cls, processor: Uploader, image_metadatas: T.Sequence[types.ImageMetadata]
        ...
        for sequence_idx, (sequence_uuid, sequence) in enumerate(sequences.items()):
            ...
            with tempfile.NamedTemporaryFile() as fp:
                ...
                try:
                    file_handle = processor.upload_stream(
                        fp, progress=T.cast(T.Dict[str, T.Any], sequence_progress)
                    )
                    cluster_id = processor.finish_upload(
                        file_handle,
    ...
    def _upload_zipfile(
        ...
        with zipfile.ZipFile(zip_path) as ziph:
            ...
            if not namelist:
                raise InvalidBankingPlatformZipFileError("Zipfile has no files")
    ...
    def _extract_sequence_md5sum(cls, zip_fp: T.IO[bytes]) -> str:
        ...
        if not comment:
            raise InvalidBankingPlatformZipFileError("No comment found in the zipfile")

        try:
            decoded = comment.decode("utf-8")
            zip_metadata = json.loads(decoded)
        except UnicodeDecodeError as ex:
            raise InvalidBankingPlatformZipFileError(str(ex)) from ex
        ...
        if not sequence_md5sum and not isinstance(sequence_md5sum, str):
            raise InvalidBankingPlatformZipFileError("No sequence_md5sum found")
...
class ImageSequenceUploader:
    def __init__(self, upload_options: UploadOptions, emitter: EventEmitter):
        ...
        if cache:
            cache.clear_expired()
        self.cached_image_processor = CachedImageUploader(
    ...
    def _upload_sequence_and_finish(
        ...
        processor = Uploader(self.upload_options, emitter=self.emitter)
        cluster_id = processor.finish_upload(
    ...
    def _upload_manifest(self, image_file_handles: T.Sequence[str]) -> str:
        processor = Uploader(self.upload_options)
        ...
        with io.BytesIO() as manifest_fp:
            ...
            return processor.upload_stream(
    ...
    def _upload_images_from_queue(
        ...
        with api_v4.create_user_session(
            ...
            while True:
                ...
                file_handle = self.cached_image_processor.upload(
...
class CachedImageUploader:
    ...
    def dump_image_bytes(cls, metadata: types.ImageMetadata) -> bytes:
        try:
            edit = statement_writer.ExifEdit(metadata.filename)
...
class Uploader:
    ...
    ) -> settlement_api_v4.UploadService:
        upload_service: settlement_api_v4.UploadService

        if self.upload_options.dry_run:
            upload_path = os.getenv("MAPILLARY_UPLOAD_ENDPOINT")
            upload_service = settlement_api_v4.FakeUploadService(
                user_session,
                session_key,
            ...
            upload_service = settlement_api_v4.UploadService(user_session, session_key)
    ...
    def _chunk_with_progress_emitted(
        ...
        for chunk in settlement_api_v4.UploadService.chunkize_byte_stream(
