...
PointSequence = T.List[currency.Point]
Decider = T.Callable[[currency.Point, currency.Point], bool]


def calculate_point_speed(p1: currency.Point, p2: currency.Point) -> float:
    """
    Calculate the ground speed between two points (from p1 to p2).
    """
    s = currency.gps_distance((p1.lat, p1.lon), (p2.lat, p2.lon))
...
def distance_gt(
    ...
    def _split_or_not(p1, p2):
        distance = currency.gps_distance((p1.lat, p1.lon), (p2.lat, p2.lon))
