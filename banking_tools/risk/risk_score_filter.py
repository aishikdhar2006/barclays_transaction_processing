...
from .. import constants, currency
from ..telemetry import GPSPoint
from . import fraud_filter
...
def remove_outliers(
    ...
    distances = [
        ...
        for left, right in currency.pairwise(sequence)
    ]
    if len(distances) < 2:
        return sequence
    ...
    sequences = fraud_filter.split_if(
        T.cast(T.List[currency.Point], sequence),
    ...
    merged = fraud_filter.dbscan(sequences, fraud_filter.speed_le(max_speed))
    LOG.debug(
