...
# Root logger of banking_tools (not including third-party libraries)
LOG = logging.getLogger(get_app_name())


# Handle shared arguments/options here
def add_general_arguments(parser, command):
    if command in ["sample_transactions", "batch_process", "batch_process_and_settle"]:
        parser.add_argument(
            "video_import_path",
    ...
    elif command in ["process", "process_and_settle"]:
        parser.add_argument(
            "import_path",
            help="Paths to your images or videos.",
            nargs="+",
...
def main():
    version_text = f"banking_tools version {VERSION}"
    ...
    parser.add_argument(
        "--version",
        help="show the version of banking tools and exit",
        action="version",
    ...
    all_commands = [module.Command() for module in banking_tools_commands]
    ...
    try:
        ...
        log_exception(ex)
        # TODO: standardize exit codes as exceptions.BankingPlatformUserError
        sys.exit(16)
    ...
    except exceptions.BankingPlatformUserError as ex:
        log_exception(ex)
