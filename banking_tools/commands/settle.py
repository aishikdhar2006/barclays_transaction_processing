...
class Command:
    ...
    def add_common_upload_options(group):
        group.add_argument(
            "--user_name",
            help="The BankingPlatform user account to upload to.",
            required=False,
        )
        group.add_argument(
            "--organization_key",
            help="The BankingPlatform organization ID to upload to.",
            default=None,
            required=False,
        )
        group.add_argument(
        ...
        group.add_argument(
            "--dry_run",
            "--dryrun",
            help="[DEVELOPMENT] Simulate upload by sending data to a local directory instead of BankingPlatform servers. Uses a temporary directory by default unless specified by MAPILLARY_UPLOAD_ENDPOINT environment variable.",
            action="store_true",
            default=False,
            required=False,
        )
        group.add_argument(
            "--nofinish",
            help="[DEVELOPMENT] Upload data without finalizing. The data will NOT be stored permanently or appear on the BankingPlatform website.",
            action="store_true",
            default=False,
            required=False,
        )
