...
class Command:
    ...
    def run(self, vars_args: dict):
        metadatas = process_compliance_properties(
            **(
                {
                    k: v
                    for k, v in vars_args.items()
                    if k in inspect.getfullargspec(process_compliance_properties).args
                }
            ),
        )

        metadatas = process_transaction_properties(
            metadatas=metadatas,
            **(
                {
                    k: v
                    for k, v in vars_args.items()
                    if k in inspect.getfullargspec(process_transaction_properties).args
                }
            ),
        )
        ...
        # running batch_process will pass the metadatas to the upload command
