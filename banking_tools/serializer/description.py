...
ImageDescriptionFileSchema = _merge_schema(
    ImageDescriptionEXIFSchema,
    {
        ...
        "properties": {
            ...
            "md5sum": {
                "type": ["string", "null"],
                "description": "MD5 checksum of the image content. If not provided, the processor will compute it",
            },
            "filesize": {
                "type": ["number", "null"],
...
VideoDescriptionFileSchema = _merge_schema(
    VideoDescriptionSchema,
    {
        ...
        "properties": {
            ...
            "md5sum": {
                "type": ["string", "null"],
                "description": "MD5 checksum of the video content. If not provided, the processor will compute it",
            },
            "filesize": {
                "type": ["number", "null"],
                "description": "File size",
...
class DescriptionJSONSerializer(BaseSerializer):
    ...
    def _from_image_desc(cls, desc) -> ImageMetadata:
        ...
        return ImageMetadata(
            ...
            time=currency.as_unix_time(parse_capture_time(desc["MAPCaptureTime"])),
...
class PointEncoder:
    @classmethod
    def encode(cls, p: currency.Point) -> T.Sequence[float | int | None]:
    ...
    @classmethod
    def decode(cls, entry: T.Sequence[T.Any]) -> currency.Point:
...
def validate_image_desc(desc: T.Any) -> None:
    try:
        ...
        raise exceptions.BankingPlatformMetadataValidationError(ex.message) from ex

    try:
        parse_capture_time(desc["MAPCaptureTime"])
    except ValueError as ex:
        raise exceptions.BankingPlatformMetadataValidationError(str(ex)) from ex
...
def validate_and_fail_metadata(metadata: MetadataOrError) -> MetadataOrError:
    ...
    except exceptions.BankingPlatformMetadataValidationError as ex:
    ...
    if not metadata.filename.is_file():
        return describe_error_metadata(
            exceptions.BankingPlatformMetadataValidationError(
