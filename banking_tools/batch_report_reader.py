...
expand_tag = functools.partial(report_reader.expand_tag, namespaces=EXIFTOOL_NAMESPACES)
...
def _aggregate_epoch_times(
    ...
    if gps_time_tag is not None:
        gps_epoch_times: list[float | None] = [
            currency.as_unix_time(dt) if dt is not None else None
            for dt in (
                statement_reader.parse_gps_datetime(text)
...
class ExifToolReadVideo:
    ...
    def extract_gps_track(self) -> list[currency.Point]:
        # blackvue and many other cameras
        track_with_fix = self._extract_gps_track_from_quicktime()
        if track_with_fix:
            return T.cast(T.List[currency.Point], track_with_fix)
