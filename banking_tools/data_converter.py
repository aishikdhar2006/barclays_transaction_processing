...
class FFMPEG:
    ...
    def __init__(
        ...
        """
        Initialize FFMPEG wrapper with paths to data_converter and ffprobe binaries.

        Args:
            data_converter_path: Path to data_converter binary executable
        ...
        """
        self.data_converter_path = data_converter_path
    ...
    def extract_frames_by_interval(
        ...
        Args:
            ...
            stream_specifier: Stream specifier to target specific stream(s).
                              ...
                              See https://data_converter.org/data_converter.html#Stream-specifiers-1

        Raises:
            FFmpegNotFoundError: If data_converter binary is not found
            FFmpegCalledProcessError: If data_converter command fails
        """
        ...
        self.run_data_converter_non_interactive(cmd)

    @classmethod
    def generate_binary_search(cls, sorted_frame_indices: list[int]) -> str:
        """
        Generate a binary search expression for data_converter select filter.
    ...
    def extract_specified_frames(
        ...
        Note:
            Frame indices are 0-based but data_converter output files are numbered starting from 1.
            ...
            try:
                ...
                cmd: list[str] = [
                    ...
                    *[
                        ...
                        *["-vsync", "0"],
                        # Set the number of video frames to output (this is an optimization to let data_converter stop early)
    ...
    def run_data_converter_non_interactive(self, cmd: list[str]) -> None:
        """
        Execute data_converter command in non-interactive mode.

        Runs data_converter with the given command arguments, automatically adding
        the -nostdin flag to prevent interactive prompts.

        Args:
            cmd: List of command line arguments to pass to data_converter

        Raises:
            FFmpegNotFoundError: If data_converter binary is not found
            FFmpegCalledProcessError: If data_converter command fails
        """
        full_cmd: list[str] = [self.data_converter_path, "-nostdin", *cmd]
        LOG.info(f"Running data_converter: {' '.join(full_cmd)}")
        try:
            subprocess.run(full_cmd, check=True, stderr=self.stderr)
        except FileNotFoundError:
            raise FFmpegNotFoundError(
                f'The data_converter command "{self.data_converter_path}" not found'
            )
