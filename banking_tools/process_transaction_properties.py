...
S = T.TypeVar("S")
R = T.TypeVar("R")
PointSequence = T.List[currency.PointLike]
...
def duplication_check(
    ...
    *,
    ...
    for cur in it:
        ...
        distance = currency.gps_distance((prev.lat, prev.lon), (cur.lat, cur.lon))

        if prev.angle is not None and cur.angle is not None:
            angle_diff = currency.diff_bearing(prev.angle, cur.angle)
        else:
            angle_diff = None
        ...
        if distance <= max_duplicate_distance and (
            ...
            ex = exceptions.BankingPlatformDuplicationError(
                msg,
...
def _check_null_island(
    ...
    for image in sequence:
        if image.lat == 0 and image.lon == 0:
            ex = exceptions.BankingPlatformNullIslandError(
...
def _interpolate_subsecs_for_sorting(sequence: PointSequence) -> None:
    ...
    Examples:
        >>> def make_point(t):
        ...     return currency.Point(lat=0, lon=0, time=t, alt=None, angle=None)
    ...
    for cur, nxt in currency.pairwise(sequence):
...
def _is_video_stationary(
    sequence: T.Sequence[currency.PointLike], max_radius_in_meters: float
) -> bool:
    if not sequence:
    ...
    for p in sequence:
        distance = currency.gps_distance(start, (p.lat, p.lon))
...
def _check_video_limits(
    ...
    for video_metadata in video_metadatas:
        try:
            ...
            if is_stationary:
                raise exceptions.BankingPlatformStationaryVideoError("Stationary video")
            ...
            if max_sequence_filesize_in_bytes is not None:
                ...
                if video_filesize > max_sequence_filesize_in_bytes:
                    raise exceptions.BankingPlatformFileTooLargeError(
            ...
            if contains_null_island:
                raise exceptions.BankingPlatformNullIslandError(
                    "GPS coordinates in Null Island (0, 0)"
                )

            avg_speed_kmh = (
                currency.avg_speed(video_metadata.points) * 3.6
            )  # Convert m/s to km/h
            too_fast = (
            ...
            if too_fast:
                raise exceptions.BankingPlatformCaptureSpeedTooFastError(
                    f"Capture speed {avg_speed_kmh:.3f} km/h exceeds max allowed {max_capture_speed_kmh:.3f} km/h",
                )
        except exceptions.BankingPlatformDescriptionError as ex:
...
def _check_sequences_by_limits(
    ...
    for sequence in input_sequences:
        avg_speed_kmh = currency.avg_speed(sequence) * 3.6  # Convert m/s to km/h
        too_fast = len(sequence) >= 2 and avg_speed_kmh > max_capture_speed_kmh

        if too_fast:
            error = exceptions.BankingPlatformCaptureSpeedTooFastError(
                f"Capture speed {avg_speed_kmh:.3f} km/h exceeds max allowed {max_capture_speed_kmh:.3f} km/h",
            )
...
def _check_sequences_zigzag(
    ...
    for sequence in input_sequences:
        ...
        for i in range(window_size, len(sequence)):
            ...
            dist_prev_curr = currency.gps_distance(
                (prev.lat, prev.lon), (curr.lat, curr.lon)
            )
            # Distance from current image back to reference
            dist_curr = currency.gps_distance((curr.lat, curr.lon), (ref.lat, ref.lon))
            # Distance from previous image to reference
            dist_prev = currency.gps_distance((prev.lat, prev.lon), (ref.lat, ref.lon))
            ...
            if (
                ...
                for j in range(i - 2, i - window_size, -1):  # Stop at ref+1
                    ...
                    dist_j_to_ref = currency.gps_distance(
                        (point_j.lat, point_j.lon), (ref.lat, ref.lon)
                    )
                    # Distance from j to curr
                    dist_j_to_curr = currency.gps_distance(
...
def process_transaction_properties(
    ...
    if image_metadatas:
        ...
        for sequence in sequences:
            if interpolate_directions:
                for image in sequence:
                    image.angle = None
            currency.interpolate_directions_if_none(sequence)
