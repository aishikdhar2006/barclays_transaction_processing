...
def sample_transactions(
    ...
    import_path: Path,
    # None if called from the sample_transactions command
    ...
    if video_start_time is not None:
        try:
            ...
            raise exceptions.BankingPlatformBadParameterError(str(ex))

    if rerun:
        for video_path in video_list:
            # Example:
            # - import_path: banking_sampled_video_frames
            # - video_dir: foo/
            # - video_path: foo/bar/zzz.mp4
            # Then:
            # - sample_dir: banking_sampled_video_frames/bar/zzz.mp4/
    ...
    for video_path in video_list:
        ...
        except data_converterlib.FFmpegNotFoundError as ex:
            # fatal error
            raise exceptions.BankingPlatformFFmpegNotFoundError(str(ex)) from ex
...
def wip_sample_dir(sample_dir: Path) -> Path:
    ...
    # prefix with .mly_data_converter_ to avoid samples being scanned by "banking_tools process"
    return sample_dir.resolve().parent.joinpath(
        f".mly_data_converter_{sample_dir.name}_{pid}_{timestamp}"
    )
...
def _sample_single_video_by_interval(
    ...
    if start_time is None:
        start_time = data_converterlib.Probe(
            data_converter.probe_format_and_streams(video_path)
        ).probe_video_start_time()
        if start_time is None:
            raise exceptions.BankingPlatformVideoError(
...
def _sample_transactions_stream_by_distance(
    points: T.Sequence[currency.Point],
    video_track_parser: format_sample_parser.TrackBoxParser,
    sample_distance: float,
) -> dict[int, tuple[format_sample_parser.Sample, currency.Point]]:
    """
    ...
    # CT is the oder of videos streaming to audiences, as well as the order data_converter sampling
