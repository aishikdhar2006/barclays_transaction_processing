...
def _is_reprocessable(metadata: types.MetadataOrError) -> bool:
    if isinstance(metadata, types.ErrorMetadata):
        if isinstance(
            metadata.error,
            (
                exceptions.BankingPlatformGeoTaggingError,
                exceptions.BankingPlatformVideoGPSNotFoundError,
                exceptions.BankingPlatformExiftoolNotFoundError,
...
def _build_image_geotag(option: SourceOption) -> base.GeotagImagesFromGeneric | None:
    ...
    if option.source in [SourceType.EXIF, SourceType.NATIVE]:
        return validate_txns_from_statement.GeotagImagesFromEXIF(
            num_processes=option.num_processes
        )

    if option.source is SourceType.EXIFTOOL_RUNTIME:
        return validate_txns_from_report.GeotagImagesFromExifToolRunner(
            num_processes=option.num_processes
        )

    elif option.source is SourceType.EXIFTOOL_XML:
        # This is to ensure 'batch_process --geotag={"source": "exiftool_xml", "source_path": "/tmp/xml_path"}'
        # to work
        if option.source_path is None:
            raise exceptions.BankingPlatformBadParameterError(
                "source_path must be provided for EXIFTOOL_XML source"
            )
        return validate_txns_from_report.GeotagImagesFromExifToolWithSamples(
            source_path=option.source_path,
            num_processes=option.num_processes,
        )

    elif option.source is SourceType.GPX:
        return validate_txns_from_audit_file.GeotagImagesFromGPXFile(
        ...
        return validate_txns_from_feed_file.GeotagImagesFromNMEAFile(
            source_path=_ensure_source_path(option),
            use_gpx_start_time=interpolation.use_gpx_start_time,
            offset_time=interpolation.offset_time,
            num_processes=option.num_processes,
        ...
        return validate_txns_from_batch.GeotagImageSamplesFromVideo(
...
def _build_video_geotag(option: SourceOption) -> base.GeotagVideosFromGeneric | None:
    ...
    Examples:
        >>> from pathlib import Path
        >>> from banking_tools.compliance.options import SourceOption, SourceType
    ...
    if option.source is SourceType.NATIVE:
        return validate_batches_from_batch.GeotagVideosFromVideo(
            num_processes=option.num_processes, filetypes=option.filetypes
        )

    if option.source is SourceType.EXIFTOOL_RUNTIME:
        return validate_batches_from_report.GeotagVideosFromExifToolRunner(
            num_processes=option.num_processes
        )
        ...
        return validate_batches_from_report.GeotagVideosFromExifToolXML(
            source_path=option.source_path,
        )

    elif option.source is SourceType.GPX:
        return validate_batches_from_audit.GeotagVideosFromGPX(
