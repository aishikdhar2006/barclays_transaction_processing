...
from .. import constants, exceptions, report_reader, types
...
class GeotagVideosFromExifToolXML(GeotagVideosFromGeneric):
    ...
    @classmethod
    def build_batch_extractors_from_etree(
        ...
        for path in video_paths:
            rdf = rdf_by_path.get(report_reader.canonical_path(path))
            if rdf is None:
                ex = exceptions.BankingPlatformExifToolXMLNotFoundError(
    ...
    @override
    def _generate_batch_extractors(
        ...
        return self.build_batch_extractors_from_etree(rdf_by_path, video_paths)
    ...
    def find_rdf_by_path(
        ...
        if option.pattern is not None:
            rdf_by_path = {}
            for path in paths:
                canonical_path = report_reader.canonical_path(path)
...
class GeotagVideosFromExifToolRunner(GeotagVideosFromGeneric):
    ...
    def _generate_batch_extractors(
        ...
        try:
            ...
            exiftool_ex = exceptions.BankingPlatformExiftoolNotFoundError(ex)
            return [
                types.describe_error_metadata(
        ...
        try:
            ...
            rdf_by_path = report_reader.index_rdf_description_by_path_from_xml_element(
                xml_element
            )

        return GeotagVideosFromExifToolXML.build_batch_extractors_from_etree(
