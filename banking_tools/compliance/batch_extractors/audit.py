...
class GPXVideoExtractor(BaseVideoExtractor):
    ...
    def extract(self) -> types.VideoMetadata:
        ...
        gpx_points: T.Sequence[currency.Point] = sum(gpx_tracks, [])

        native_extractor = NativeVideoExtractor(self.video_path)

        try:
            native_video_metadata = native_extractor.extract()
        except exceptions.BankingPlatformVideoGPSNotFoundError as ex:
    ...
    def _gpx_offset(
        ...
        >>> gpx_points = [currency.Point(time=5, lat=1, lon=1, alt=None, angle=None)]
