...
class ImageEXIFExtractor(BaseImageExtractor):
    ...
    def _exif_context(self) -> T.Generator[statement_reader.ExifReadABC, None, None]:
        with self.image_path.open("rb") as fp:
            yield statement_reader.ExifRead(fp)
        ...
        with self._exif_context() as exif:
            ...
            if lonlat is None:
                if not self.skip_lonlat_error:
                    raise exceptions.BankingPlatformGeoTaggingError(
            ...
            image_metadata = types.ImageMetadata(
                ...
                time=currency.as_unix_time(capture_time),
                lat=lat,
                lon=lon,
