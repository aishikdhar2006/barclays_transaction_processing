...
from . import constants, exceptions, statement_writer, types, utils
from .compliance.factory import parse_source_option, process
from .compliance.options import (
    InterpolationOption,
    SourceOption,
    SourcePathOption,
    SourceType,
...
from .serializer.report import GPXSerializer
...
def process_compliance_properties(
    ...
    video_geotag_source: list[str],
    # Global options
    # video_import_path comes from the command batch_process
...
def _overwrite_exif_tags(
    ...
    for metadata in tqdm(
        ...
        try:
            image_exif = statement_writer.ExifEdit(metadata.filename)
...
def _show_stats(
    ...
    if critical_error_metadatas:
        raise exceptions.BankingPlatformProcessError(
...
def process_finalize(
    ...
    # process_and_settle will set desc_path to "\x00"
    ...
    if skip_process_errors:
        ...
        skipped_process_errors = {exceptions.BankingPlatformDuplicationError}
