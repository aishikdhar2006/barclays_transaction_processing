...
from banking_tools.statement_reader import (
    _parse_coord,
    ExifRead,
    parse_datetimestr_with_subsec_and_offset,
)
from banking_tools.statement_writer import ExifEdit
...
def test_read_and_write(setup_data: py.path.local):
    ...
    for dt in dts:
        ...
        assert actual
        assert currency.as_unix_time(dt) == currency.as_unix_time(actual), (dt, actual)
    ...
    for dt in dts:
        ...
        assert actual
        assert currency.as_unix_time(dt) == currency.as_unix_time(actual)
...
class TestExtractCameraUuidFromEXIF:
    ...
    def test_body_serial_only(self):
        ...
        from banking_tools.statement_reader import ExifReadFromEXIF
...
class TestExtractCameraUuidFromXMP:
    ...
    def _create_xmp_reader(self, tags_dict: dict):
        ...
        from banking_tools.statement_reader import ExifReadFromXMP, XMP_NAMESPACES
...
class TestVideoExtractCameraUuid:
    ...
    def _create_video_statement_readerer(self, tags_dict: dict):
        """Helper to create an ExifToolReadVideo with mocked tags"""
        from banking_tools.batch_report_reader import (
            ExifToolReadVideo,
            EXIFTOOL_NAMESPACES,
        )
        import xml.etree.ElementTree as ET
...
class TestExifToolReadExtractCameraUuid:
    ...
    def _create_report_readerer(self, tags_dict: dict):
        """Helper to create an ExifToolRead with mocked tags"""
        from banking_tools.report_reader import ExifToolRead, EXIFTOOL_NAMESPACES
        import xml.etree.ElementTree as ET
    ...
    def test_generic_serial_fallback(self):
        ...
        reader = self._create_report_readerer({"ExifIFD:SerialNumber": "GENERIC123"})
    ...
    def test_ifd0_serial_fallback(self):
        ...
        reader = self._create_report_readerer({"IFD0:SerialNumber": "IFD0SN123"})
    ...
    def test_xmp_aux_serial(self):
        ...
        reader = self._create_report_readerer({"XMP-aux:SerialNumber": "AUXSN456"})
