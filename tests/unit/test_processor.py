...
import py.path
import pytest
from banking_tools import api_v4, processor
...
def test_upload_images(setup_unittest_data: py.path.local, setup_upload: py.path.local):
    ...
    results = list(
        processor.ZipUploader.zip_images_and_upload(
            mly_processor,
            [
...
def test_upload_zip(
    ...
    processor.ZipUploader.zip_images(
        [
            description.DescriptionJSONSerializer.from_desc(T.cast(T.Any, desc))
            for desc in descs
        ],
    ...
    for zip_path in zip_dir.listdir():
        cluster = processor.ZipUploader._upload_zipfile(mly_processor, Path(zip_path))
        assert cluster == "0"
    actual_descs = sum(extract_all_uploaded_descs(Path(setup_upload)), [])
...
def test_upload_blackvue(tmpdir: py.path.local, setup_upload: py.path.local):
    ...
    with Path(blackvue_path).open("rb") as fp:
        file_handle = mly_processor.upload_stream(
            fp,
            session_key="this_is_a_blackvue.mp4",
        )
    cluster_id = mly_processor.finish_upload(
...
class TestImageSequenceUploader:
    ...
    def test_image_sequence_processor_basic(self, setup_unittest_data: py.path.local):
        ...
        # Test upload
        results = list(sequence_processor.upload_images(image_metadatas))
    ...
    def test_image_sequence_processor_multithreading_with_cache_enabled(
        ...
        upload_options_with_cache = processor.UploadOptions(
            {"user_upload_token": "YOUR_USER_ACCESS_TOKEN"},
            upload_cache_path=Path(setup_unittest_data.join("upload_cache")),
        ...
        sequence_processor.upload_options = dataclasses.replace(
            upload_options_with_cache, dry_run=True
        )
        sequence_processor.cached_image_processor.upload_options = dataclasses.replace(
            upload_options_with_cache, dry_run=True
        )

        # Verify cache is available and shared
        assert sequence_processor.cached_image_processor.cache is not None, (
    ...
    def test_image_sequence_processor_multithreading_with_cache_disabled(
        self, setup_unittest_data: py.path.local
    ):
        ...
        with patch("banking_tools.constants.UPLOAD_CACHE_DIR", None):
            upload_options = processor.UploadOptions(
                {"user_upload_token": "YOUR_USER_ACCESS_TOKEN"},
                upload_cache_path=Path(setup_unittest_data.join("upload_cache")),
                num_upload_workers=4,  # This will be used internally for parallel image uploads
            ...
            retrieved_value = (
                sequence_processor.cached_image_processor._get_cached_file_handle(
                    test_key
                )
            )
            ...
            sequence_processor.cached_image_processor._set_file_handle_cache(
                test_key, test_value
            )
    ...
    def test_image_sequence_processor_cache_hits_second_run(
        self, setup_unittest_data: py.path.local
    ):
        ...
        cache_enabled_options = processor.UploadOptions(
            {"user_upload_token": "YOUR_USER_ACCESS_TOKEN"},
            upload_cache_path=Path(setup_unittest_data.join("upload_cache")),
            dry_run=False,  # Cache requires dry_run=False initially
        )

        # Create the sequence processor - your changes now automatically expose cache
        emitter = processor.EventEmitter()
        sequence_processor = processor.ImageSequenceUploader(
        ...
        assert list(sequence_processor.cached_image_processor.cache.keys()) == []
        results_1 = list(
            sequence_processor.upload_images([images["a"], images["b"], images["c"]])
        )
        ...
        first_upload_cache_keys = set(
            sequence_processor.cached_image_processor.cache.keys()
        )
        assert len(first_upload_cache_keys) == 3

        results_2 = list(
            sequence_processor.upload_images(
                [
    ...
    def test_image_sequence_processor_multiple_sequences(
        self, setup_unittest_data: py.path.local
    ):
    ...
    def test_image_sequence_processor_event_emission(
        self, setup_unittest_data: py.path.local
    ):
