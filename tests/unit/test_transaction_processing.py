...
from banking_tools import (
    exceptions,
    currency,
    process_compliance_properties as pgp,
    process_transaction_properties as psp,
    types,
)
...
def test_find_sequences_by_folder(tmpdir: py.path.local):
    ...
    metadatas = psp.process_transaction_properties(
        sequence,
    ...
    for s in actual_metadata.values():
        for c, n in currency.pairwise(s):
...
def test_process_finalize(setup_data):
    ...
    expected = [
        ...
        {
            "error": {
                "type": "BankingPlatformMetadataValidationError",
...
def test_video_error(tmpdir: py.path.local):
    sequence: T.List[types.Metadata] = [
        types.VideoMetadata(
            ...
            points=[
                currency.Point(1, -0.00001, -0.00001, 1, angle=None),
                currency.Point(1, 0, 0, 1, angle=None),
                currency.Point(1, 0.00010, 0.00010, 1, angle=None),
            ],
            make="hello",
            model="world",
            filesize=123,
        ...
        types.VideoMetadata(
            ...
            types.FileType.VIDEO,
            points=[
                currency.Point(1, 1, 1, 1, angle=None),
                currency.Point(1.1, 1.00001, 1.00001, 1, angle=None),
                currency.Point(10, 1, 3, 1, angle=None),
            ],
            make="hello",
            model="world",
            filesize=123,
        ...
        types.VideoMetadata(
            ...
            points=[
                currency.Point(1, 1, 1, 1, angle=None),
                currency.Point(2, 1.0002, 1.0002, 1, angle=None),
            ],
            make="hello",
            model="world",
    ...
    assert isinstance(
        ...
        exceptions.BankingPlatformNullIslandError,
    )
    assert isinstance(
...
def test_image_null_island(tmpdir: py.path.local):
    ...
    assert isinstance(
        ...
        exceptions.BankingPlatformNullIslandError,
    ), "Error should be BankingPlatformNullIslandError"
...
def test_split_sequence_by_filesize(tmpdir):
    ...
    metadatas = psp.process_transaction_properties(sequence)
...
def test_zigzag_detection_deviation(tmpdir: py.path.local):
    ...
    zigzag_errors = [
        ...
        if isinstance(d.error, exceptions.BankingPlatformZigZagError)
    ]
...
def test_zigzag_detection_skip_flag(tmpdir: py.path.local):
    ...
    metadatas = psp.process_transaction_properties(sequence, skip_zigzag_check=True)
    image_metadatas = [d for d in metadatas if isinstance(d, types.ImageMetadata)]
    zigzag_errors = [
        d
        for d in metadatas
        if isinstance(d, types.ErrorMetadata)
        and isinstance(d.error, exceptions.BankingPlatformZigZagError)
    ]
...
def test_capture_speed_exceeds_limit(tmpdir: py.path.local):
    ...
    assert all(
        isinstance(d.error, exceptions.BankingPlatformCaptureSpeedTooFastError)
        for d in error_metadatas
    )
