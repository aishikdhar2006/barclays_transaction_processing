...
from banking_tools.risk import fraud_filter
from banking_tools.risk.risk_score_filter import remove_noisy_points, remove_outliers
from banking_tools.telemetry import GPSFix, GPSPoint
...
class TestSplitIf:
    def test_empty_list(self):
        assert fraud_filter.split_if([], lambda a, b: True) == []

    def test_single_point(self):
        p = _make_point(0.0, 0.0, 0.0)
        result = fraud_filter.split_if([p], lambda a, b: True)
        assert len(result) == 1
        assert result[0] == [p]

    def test_no_splits(self):
        points = [_make_point(float(i), 0.0, 0.0) for i in range(5)]
        result = fraud_filter.split_if(points, lambda a, b: False)
        assert len(result) == 1
        assert len(result[0]) == 5

    def test_split_every_point(self):
        points = [_make_point(float(i), 0.0, 0.0) for i in range(5)]
        result = fraud_filter.split_if(points, lambda a, b: True)
        assert len(result) == 5
        for seq in result:
            assert len(seq) == 1
...
class TestSpeedLe:
    def test_slow_speed_true(self):
        decider = fraud_filter.speed_le(1000)
        p1 = _make_point(0.0, 48.0, 11.0)
        p2 = _make_point(10.0, 48.001, 11.001)
        assert decider(p1, p2) is True

    def test_fast_speed_false(self):
        decider = fraud_filter.speed_le(0.001)
        p1 = _make_point(0.0, 0.0, 0.0)
        p2 = _make_point(1.0, 1.0, 1.0)
        assert decider(p1, p2) is False
...
class TestUpperWhiskerEdge:
    def test_raises_on_single_value(self):
        with pytest.raises(statistics.StatisticsError):
            fraud_filter.upper_whisker([1])

    def test_even_length(self):
        # [1, 2, 3, 4] -> q1=1.5, q3=3.5, irq=2, upper=3.5+3=6.5
        assert fraud_filter.upper_whisker([1, 2, 3, 4]) == 6.5

    def test_odd_length(self):
        # [1, 2, 3, 4, 5] -> q1=median([1,2])=1.5, q3=median([4,5])=4.5, irq=3, upper=4.5+4.5=9.0
        assert fraud_filter.upper_whisker([1, 2, 3, 4, 5]) == 9.0


# --- Tests for risk_score_filter module ---
