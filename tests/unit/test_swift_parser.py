...
from banking_tools import swift_parser, telemetry
from banking_tools.formats import construct_format_parser as cparser
...
[1623057130221]$GPGSA,A,3,19,02,06,12,17,09,05,20,04,25,,,1.65,0.82,1.43*08
    ...
    info = swift_parser.extract_blackvue_info(io.BytesIO(data))
    assert info == swift_parser.BlackVueInfo(
        gps=[
            telemetry.GPSPoint(
                time=0.0,
...
def test_gpspoint_gga():
    ...
    points = swift_parser._parse_gps_box(gps_data)
