...
import gpxpy
from banking_tools import constants, currency, telemetry
from banking_tools.risk import fraud_filter

from .risk_parser import _convert_points_to_gpx_track_segment
...
def _gpx_track_segment_to_points(
    ...
    for p in segment.points:
        ...
        point = telemetry.GPSPoint(
            time=currency.as_unix_time(T.cast(datetime.datetime, p.time)),
            lat=p.latitude,
            lon=p.longitude,
            alt=p.elevation,
            angle=None,
...
def _filter_outliers(
    ...
    distances = [
        currency.gps_distance((left.lat, left.lon), (right.lat, right.lon))
        for left, right in currency.pairwise(points)
    ]
    if len(distances) < 2:
        return points

    max_distance = fraud_filter.upper_whisker(distances)
    max_distance = max(gps_precision + gps_precision, max_distance)

    subseqs = fraud_filter.split_if(
        T.cast(T.List[currency.Point], points),
        fraud_filter.distance_gt(max_distance),
    )
    ...
    max_speed = fraud_filter.upper_whisker(ground_speeds)
    merged = fraud_filter.dbscan(subseqs, fraud_filter.speed_le(max_speed))

    return T.cast(
        T.List[telemetry.GPSPoint],
        fraud_filter.find_majority(merged.values()),
    )
