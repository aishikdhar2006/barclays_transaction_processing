...
import gpxpy
import gpxpy.gpx
from banking_tools import currency, utils
from banking_tools.ledger import ledger_parser
...
def main():
    ...
    video_paths = utils.find_videos(
        [pathlib.Path(p) for p in parsed_args.ledger_video_path]
    )
    ...
    if parsed_args.imu:
        ...
        for path in video_paths:
            with path.open("rb") as fp:
                camm_info = ledger_parser.extract_camm_info(fp, telemetry_only=True)
