...
run_batch_process_for_descs = functools.partial(
    run_process_for_descs, command="batch_process"
)

run_sample_transactions = functools.partial(run_command, command="sample_transactions")


def test_sample_transactions_relpath():
    pytest_skip_if_not_data_converter_installed()
...
def test_sample_transactions_without_video_time(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()

    video_dir = setup_data.join("videos")
    root_sample_dir = video_dir.join("banking_sampled_video_frames")
...
def test_sample_transactions_specify_video_start_time(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()
...
def test_batch_process(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()
    ...
    descs = run_batch_process_for_descs(
        [
            "--video_sample_interval=2",
            "--video_sample_distance=-1",
...
def test_batch_process_sample_with_multiple_distances(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()
...
def test_batch_process_sample_with_distance(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()
...
def test_batch_process_multiple_videos(setup_data: py.path.local):
    pytest_skip_if_not_data_converter_installed()
