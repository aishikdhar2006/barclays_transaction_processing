...
from .fixtures import (
    ...
    run_process_and_settle_for_descs,
    setup_config,
    setup_data,
    setup_upload,
)
...
def test_process_and_settle(setup_data: py.path.local, setup_upload: py.path.local):
    input_paths = [
    ...
    descs = run_process_and_settle_for_descs([*[str(path) for path in input_paths]])
    assert_contains_image_descs(
        descs,
...
@pytest.mark.usefixtures("setup_config")
def test_process_and_settle_images_only(
    setup_data: py.path.local, setup_upload: py.path.local
):
    descs = run_process_and_settle_for_descs(
        [
            "--filetypes=image",
...
@pytest.mark.usefixtures("setup_config")
def test_batch_process_and_settle(
    ...
    run_process_and_settle_for_descs(
        ...
        command="batch_process_and_settle",
    )
...
@pytest.mark.usefixtures("setup_config")
def test_batch_process_and_settle_after_gpx(
    setup_upload: py.path.local, setup_data: py.path.local
):
    pytest_skip_if_not_data_converter_installed()
