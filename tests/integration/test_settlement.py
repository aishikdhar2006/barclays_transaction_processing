...
def test_upload_everything(setup_data: py.path.local, setup_upload: py.path.local):
    descs = run_process_and_settle_for_descs([str(setup_data)])
...
def test_upload_image_dir(setup_data: py.path.local, setup_upload: py.path.local):
    descs = run_process_and_settle_for_descs(["--file_types=image", str(setup_data)])
...
def test_upload_image_dir_twice(setup_data: py.path.local, setup_upload: py.path.local):
    ...
    # first upload
    first_descs = run_process_and_settle_for_descs(
    ...
    second_descs = run_process_and_settle_for_descs(
        [
            "--file_types=image",
            str(setup_data),
            str(setup_data),
...
def test_upload_wrong_descs(setup_data: py.path.local, setup_upload: py.path.local):
    ...
    desc_path = setup_data.join("banking_image_description.json")
